...
with client.session(profile_id="my-session") as session:
    ...
    # Note: run_task preserves browser state (like highlighting or current URL)
    # but starts with a fresh agent memory.
    session.run_task("Find the top story and remember its title")
    
    # 3. If you want to use information from a previous task, 
    # you can pass it in the prompt explicitly.
    last_result = session.result().result
    session.run_task(f"The previous story was '{last_result}'. Now find the second story and compare them.")
...
<ResponseField name="close" type="method">
  Closes the session manually. Only necessary if not using a context manager.
</ResponseField>
