...
with client.session(profile_id="my-session") as session:
    ...
    result = session.run_task("Find the top story and remember its title")
    
    # 3. Use information from the previous task result
    session.run_task(f"The previous story was '{result.result}'. Now find the second story and compare them.")

# After the session is closed, you can access the final result and recording
print(f"Final session result: {session.result().result}")
...
async def main():
    ...
    # Always close the client when done if not using a context manager
    await client.close()
