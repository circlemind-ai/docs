...
<ResponseField name="response_model" type="dict | BaseModel">
  ...
  <Note>
    If you set `response_model`, Smooth will see it, no need to include it in `task`.
  </Note>
</ResponseField>
...
<ResponseField name="metadata" type="string">
  A map containing variables or parameters that will be passed to the agent.
...
<Warning>
  `TaskResponse` contains also `live_url` and `recording_url` to be compatible with the API structure.
  Use `TaskHandle.live_url()` and `TaskHandle.recording_url()` as they handle the underlying API polling for you.
</Warning>
...
Smooth supports structured output, simply set the `response_model` field by passing either a Pydantic Model or a JSON schema.
