...
<ResponseField name="additional_tools" type="dict">
  Additional tools to enable for the task. Each tool is to a `{tool_name: tool_kwargs} pair.

  Currently supported tools are:
  | Tool | Params | Purpose |
  |------|--------|---------|
  |`screenshot`| - `full_page (boolean)`: whether to take a full page screenshot rather than just the viewport. Default is False | Allows Smooth to take screenshot of the browser viewport |
