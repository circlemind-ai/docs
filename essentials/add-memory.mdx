---
title: 'Add Memories'
description: 'Add Memories'
icon: 'brackets-curly'
---

Before adding memories, make sure to [configure the API key in your environment](/essentials/getting-started#getting-started).

You can add memories to a graph by using the `add` method. Add currently supports:
- **Raw Text**: chunking will be handled automatically;
- **PDFs**: parsing and chunking will be handled automatically;
- ...and more coming soon!

#### Adding text

```python
  from circlemind import Circlemind

  # Initialize the client
  client = Circlemind()

  # Add a memory
  client.add(
    memory="Sophie enjoys hiking in the Italian Alps.",
    graph_id="customer-id-123"
  )
```

The `graph_id` parameter is optional. If not set, it defaults to the `default` graph.

#### Adding PDFs

```python
  from pathlib import Path

  # Add a file
  client.add(
    memory=Path("path/to/file.pdf"),
    graph_id="customer-id-123"
  )
```

### Metadata

You can also specify `metadata` to pass a dictionary of information that would be tied to the memory you're creating.
For example, this can be the document URL that can be used, at query time, to [reference the sources used to produce an answer](/essentials/query#include-references).

```python
  from pathlib import Path

  # Add a file
  client.add(
    memory=Path("path/to/file.pdf"),
    graph_id="customer-id-123",
    metadata={"url": "https://path/to/file.pdf"}
  )
```