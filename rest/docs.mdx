...
All requests, except [add](https://api.circlemind.co/redoc#operation/post_insert_graph__graph_name__insert_post) and [query](https://api.circlemind.co/redoc#operation/post_query_graph__graph_name__query_post), immediately return a response.
...
In order to perform a memory addition, we need to send an `add` request to the workers. The processing is asynchronous and can be optionally [tracked](https://api.circlemind.co/redoc#operation/get_insert_status_graph__graph_name__insert__request_id__get) until completion.
...
For more detailed information on each endpoint, refer to the full API reference at [https://api.circlemind.co/redoc](https://api.circlemind.co/redoc).